...
## Read in a cands.astrom file [produced by MOP::find.pl] and
## create mpc format files for each object in the cand.comb 
## file
##
...
foreach my $line  ( @{$cand_file->{ARRAY}} ) {
	my $mpc;
	my $obj_name=sprintf "%s%s%s", $obj,$obj_letters[$num_obj],$chip_letter;
	$obj_name =~ s/^A/o/;
	warn "OBJECT NAME: $obj_name\n" if ( !$quiet) ;
	foreach my $frame ( @frames ) {
	    my $filter = `gethead $frame.fits FILTER`;
	    chomp $filter;
	    my %this =  %{$line->{$frame}};
	    $filter=~m/^(\S)/;
	    $this{FILTER}=$1;
	    ...
	    print "$cmd\n" if ($debug);
	    my $check;
	    unlink ('dophot.pl.OK') if -e 'dophot.pl.OK';
	    unlink ('dophot.pl.FAILED') if -e 'dophot.pl.FAILED';
	    ...
	    my ( $rsign, @ra, $dsign, @dec);
	    slaCr2tf(2, $this{"R.A."}*DD2R, $rsign, @ra );
	    slaCr2af(1, $this{DEC}*DD2R, $dsign, @dec);

	    $this{"R.A."} = sprintf("%02d:%02d:%02d.%02d",@ra);
	    $this{DEC} = sprintf("$dsign%02d:%02d:%02d.%01d",@dec);
	    $this{MAG} = $this{MAG} - $cand_file->{HDU}{$frame}{APCOR};
	    $mpc .= sprintf "#L %s [provisional]\n", $frame;
	    $mpc .= sprintf " %-11.11s  C%16.16s %11.11s %11.11s         %5.1f %1s      568\n",
	    $obj_name,$cand_file->{HDU}{$frame}{'MJD-OBS-CENTER'},$this{"R.A."},$this{DEC},$this{MAG},$this{FILTER};
	}
