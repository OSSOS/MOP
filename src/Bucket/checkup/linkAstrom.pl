...
sub fuse {
    ...
    my $sql = "SELECT distinct(floor(max(mjdate)/$day_frac)) date FROM ";
    $sql .= " measure m LEFT JOIN object o ON m.provisional LIKE o.provisional ";
    $sql .= " WHERE o.official IS NULL AND mjdate>51911 group by m.provisional order by date desc";
    ...
    while ( my $row = $sth->fetchrow_hashref ) {
        my $this_mjdate=$row->{'date'}*$day_frac;	
	my ($raRAD, $decRAD, $dra, $ddec, $ang ) = Bucket::MOP::predict($abg_filename,$this_mjdate-$day_frac,568);
	my ($raRAD2, $decRAD2, $dra2, $ddec2, $ang2 ) = Bucket::MOP::predict($abg_filename,$this_mjdate+$day_frac,568);
	...
	my %select = ( "WHERE" => "$dangle < 5.0*$err_circ AND mjdate < $this_mjdate+$day_frac and mjdate > $this_mjdate-$day_frac ",
		       "GROUP" => "provisional");
	...
	while ( my $res=$measure->fetchRow ) {
	    ...
	    print STDERR "Checking $measure->{provisional} on $this_mjdate +/- $day_frac                                 \r";
...
sub new_object {
    ...
    $sql .= " WHERE o.official IS NULL AND m.provisional RLIKE '^d4' ";
...
my $obj_sql="SELECT official, count(*) nobs FROM object o JOIN measure m ON o.provisional LIKE m.provisional $where GROUP BY o.official having count(*)< 16 order by nobs desc";
