...
$ENV{"SEX_CONFIG"} ||  $ENV{"SEXCONFIG"} ||
    die "Define the environment variable SEX_CONFIG to indicate where the sextract parameter files are stored.";

my $configfile='default.sex';
if ( $ENV{"SEX_CONFIG"} ) {
    $configfile = "$ENV{SEX_CONFIG}/default.sex";
}

if ( $ENV{"SEXCONFIG"})  {
    $configfile = "$ENV{SEXCONFIG}/default.sex";
}
