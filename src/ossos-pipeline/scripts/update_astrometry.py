...
def recompute_mag(mpc_obs):
    ...
    reading = astrom.SourceReading(float(mpc_obs.comment.x), float(mpc_obs.comment.y), float(mpc_obs.comment.x),
                                   ...
                                   mpc_obs.coordinate.dec.degrees, float(mpc_obs.comment.x), float(mpc_obs.comment.y),
...
def main(mpc_file, cor_file):
    out_mags = []
    out_merrs = []
    in_mags = []
    in_merrs = []
    fptr = storage.open_vos_or_local(mpc_file)
    optr = storage.open_vos_or_local(cor_file, mode='w')
...
if __name__ == '__main__':
    ...
    main(args.mpc_file, args.cor_file)
