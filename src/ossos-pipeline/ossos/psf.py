...
class PSF(object):
    ...
    def __init__(self, expnum, ccd, version='p', prefix=None, fwhm=None, zeropoint=None):
        self.expnum = expnum
        self.ccd = ccd,
        self.version = type
    ...
    def zeropoint(self):
        if self._zeropoint is None:
            try:
                self._zeropoint = storage.get_zeropoint(**self.kwargs)
            except:
                base_image = storage.get_image(return_file=False, **self.kwargs)
                zeropoint = base_image.header.get()


    @property
    def apmin(self):
        return SMALL_APERTURE_FRACTION*self.fwhm

    @property
    def apmax(self):
        return LARGE_APERTURE_FRACTION*self.fwhm

    def build_psf(self):
        logging.info("Builing PSF: {}".format(self.filename))
        bright_star_list = storage.get_file(ext=BRIGHT_STARLIST_EXTENSION, **self.kwargs)
