...
c
c-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
c This subroutine fits the given PSF to a set of stars.
c-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-

      SUBROUTINE  NSTAR  (PAR, MAXPAR, PSF, MAXPSF, MAXEXP, DATA, 
     $  NCOL, NROW, FITRAD, E1, E2, psf_head, lobad, hibad, phpadu,
     $  ronois)
C
C=======================================================================
C
...
C
     ...
     .     EXTEND
      ...
      INTEGER IX, IY, I3, I3M2, L, NI, IFAINT
...
C
      read (psf_head(1), '(1x, a8, 4i5, f9.3, f15.3, 2f9.1)')
     $  label, npsf, npar, nexp, nfrac, psmag, bright, xpsf, ypsf
      do ipstyp = 1, maxtyp
         i = nparam(ipstyp, 1., check, par, maxpar)
         if ((label .eq. check) .and. (i .eq. npar)) goto 1100
      end do
      write (6, *) 'Inappropriate PSF: '//label
      stop

 1100 continue
      read (psf_head(2), *) (par(i), i=1,npar)
      PERERR = 0.01 * E1
      PKERR = 0.01 * E2 / (PAR(1)*PAR(2)) ! **2
C
...
C
      CALL RDHEAD (2, NL, IDUM, IDUM, LOBAD, HIBAD, 
     .     PHPADU, RONOIS, DUM)
...
C
      ...
      END IF
C
      CALL WRHEAD (1, 1, NCOL, NROW, 7, LOBAD, HIBAD, 
