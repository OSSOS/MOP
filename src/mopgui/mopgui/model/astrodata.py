...
ASTRODATA_MSG_NEXT_OBS = ASTRODATA_MSG_ROOT + ("nextobs",)
ASTRODATA_MSG_PREV_OBS = ASTRODATA_MSG_ROOT + ("prevobs",)
...
class AstroDataModel(object):
    ...
    def next_source(self):
        ...
        pub.sendMessage(ASTRODATA_MSG_NEXT_SRC, data=self._current_src_number)

    def previous_source(self):
        self._current_src_number = (self._current_src_number - 1) % self.get_source_count()
        pub.sendMessage(ASTRODATA_MSG_PREV_SRC, data=self._current_src_number)
    ...
    def next_obs(self):
        ...
        pub.sendMessage(ASTRODATA_MSG_NEXT_OBS, data=self._current_obs_number)

    def previous_obs(self):
        self._current_obs_number = (self._current_obs_number - 1) % self.get_obs_count()
        pub.sendMessage(ASTRODATA_MSG_PREV_OBS, data=self._current_obs_number)
