#  ptrail.cl  modification of pcfh00I.cl 
# 
# Modification history:
# 
# Jan 10/01 Gladman: Modified for VLT 2000 data
# 
# Nov 13/00 Gladman: fixed up for July 1999 CFHT Uran/Neptune search
#
...
procedure ptrail (common,prefimg,pfieldn,pseed)
...
#	JJK:  replaced with a read of naxis[1] and naxis[2]
...
# xref yref and xpst ypst are for the x,y shifts of non-aligned images JJK
	...
	nobjdis = pnobjdis
#	frmsz = pfrmsz
#	Get the x size and y size seperately JJK 28/03/2000
	infile = rootword//1
	...
	gettime(infile,fieldn)
        endtime = gettime.outputime
        deltatime = endtime - strttime

	if (nobjmin > 250) {nobjdis = 250}
	if (nobjmin+nobjdis > 250) {nobjdis = 251 - nobjmin}
        print(" ")
	...
	j = nobjmin+nobjdis-1
        ...
        print(" ") 
        print(" Objects move to lower right! ") 
        print(" ") 
...
# Add read of XCENTER and YCENTER to allow non-alligned images
	...
	list=tfile
	dummy = fscan(list,xref,yref,refmag)
	delete(tfile, verify-)
	    ...
	    tfile=mktemp("tmp$pt")
            ...
            list=tfile
            dummy = fscan(list,xpst,ypst,thisrefmag)
	    dx = xref - xpst
	    dy = yref - ypst
	    print (dx,dy) ;
#	    delete(tfile, verify-)
...
# This is a sloppy re-use of the objfile variable....but it's OK.
            ...
            for (i=1; i<=nobj; i+=1) {
              xsh0 = rs[i]*cos(as[i]) ;
# A negative sign before rs[i] in the following produces decreasing y motion
              ysh0 = rs[i]*sin(as[i]) ;
...
# Only include if the object does fall off the edge of the frame
