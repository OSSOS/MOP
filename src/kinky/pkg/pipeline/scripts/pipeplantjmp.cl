...
## Internal varibles.
	...
	real dx,dxx,dxy,dy,dyx,dyy,dmag,delta,nx,ny;
	int  id, count, nsteps;
	real npix,fwhm;
	real dtime,dt,itime;
	bool plant
...
### Load the linear transformation + dmag
	  sx = "#";
	  while ( fscan(pipeshifts,sx,dxx,dxy,dy,dyx,dyy,dmag)!=EOF ) {
...
# Apply the linear transformation
	    delta = dxx*dyy - dxy*dyx
	    nx =  (dyy*(x - dx) + dxy*(y - dy))/delta;
	    ny =  (dxx*(y - dy) + dyx*(x - dx))/delta;
	    x = nx;
	    y = ny;
	    mag = mag + dmag;
