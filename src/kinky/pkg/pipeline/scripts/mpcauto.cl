# (MPC)-producing AUTO photometry script intended for CFHT pipeline

procedure mpcauto(pin)
...
# 10 DEC 2002 : JJK fixxed rounding error.  Changed to make autonomous.

string pin  {"FILE.cands.astrom", prompt="file with header and x/y/RA/DEC"}
string apfile {"fkaper.corr", prompt="file containing the apperature corrections"}
   ...
   gparams:
	...
	for(j=1; j<=3; j+=1){
	   dummy=fscan(list,ac1r,ac2r,acr)
           # print(" >>> acin,acout,ac : ", ac1r,ac2r,acr)
	   acin[j] = ac1r
	   acout[j] = ac2r
	   ac[j] = acr
	}
...
#       ------- diagnose photometry and store strings -----------
	if (tmag < 8.0 || tmag > 30.0) goto photfail
        ...
        mag[k] = tmag - ac[k]
        if ( ( 10.0*mag[k]-int(10.0*mag[k]) ) > 0.500 ) {mag[k]=mag[k]+0.05}
        smag = str(mag[k])
