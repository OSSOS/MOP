...
procedure plantvlt (common,prefimg,pfieldn,pseed)
	...
	real pnobjdis  {"", prompt=" max number MORE than min number"}
        real pfrmsz   {1900, prompt=" frame size (pixels) SQUARE!"}
...
begin 
	...
	real   xss[250],yss[250],as[250],rs[250],mags[250]
	real   dx,dy,dist
	real   rmag,frmsz
	...
	if (nobjmin+nobjdis > 251) {nobjdis = 251 - nobjmin}
        print(" ")
...
# Note that this is now a dummy read to allow operator to abort...
        scan(cpflag)
	cpflag=1
