...
###  ---- yupper and Ylower set the 'trim' area so that we can exclude candidates from the 
###  trim section of the images.
###  that happens just after the dupflag if statments where the astrom file is being read
###  object by object ::: : JJK
        yupper=4612
        ylower=1
        if ( int(chipid)<19 ) { 
           ylower=4644-yupper
...
#                            ---- tolerance is 3-pix radius ---
		if( ((diffx*diffx + diffy*diffy) < 9.0) && dupflag==0 ) {
			...
			dupflag = 1
             ...
             if (  dupflag==0 && ( x1 < 33 || x1 > 2080 || y1 < ylower || y1 > yupper)  ) { 
                 newflag[nread]=0
                 nnew = nnew - 1
	         dupflag = 1
             }
