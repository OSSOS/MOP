...
do
   vls ${DBIMAGES}/${expnum}>&/dev/null || ( vmkdir -p ${DBIMAGES}/${expnum} ; vln $asgwyn/${expnum}p.fits ${DBIMAGES}/${expnum}/${expnum}p.fits )
done 
...
do
	...
	mk_mopheader ${exp1} ${exp2} ${exp3} -v --ccd ${ccd} ${force} --dbimages ${DBIMAGES} ${dry_run} --ignore-update-headers
	...
	mk_mopheader ${exp1} ${exp2} ${exp3} -v --type s --ccd ${ccd} ${force} --dbimages ${DBIMAGES} ${dry_run} --ignore-update-headers
	mkpsf ${exp1} ${exp2} ${exp3} --ccd ${ccd} -v --type s --dbimages ${DBIMAGES}  ${force} ${dry_run}
	...
	mk_mopheader ${exp1} ${exp2} ${exp3} -v --type s --fk --ccd ${ccd} ${force} --dbimages ${DBIMAGES} ${dry_run} --ignore-update-headers
	mkpsf ${exp1} ${exp2} ${exp3} --ccd ${ccd} --fk -v --type s --dbimages ${DBIMAGES} --type s ${force} ${dry_run}
	...
	astrom_mag_check ${field} ${ccd} --measure3 ${MEASURE3}  --dbimages ${DBIMAGES}  --force ${dry_run} --fk
    ...
    # Now run the standard pipeline on the artificial sources.
    mkpsf ${exp1} ${exp2} ${exp3} --ccd ${ccd} --fk -v --type s --dbimages ${DBIMAGES} --type s ${force}
    ...
    astrom_mag_check ${field} ${ccd} --measure3 ${MEASURE3}  --dbimages ${DBIMAGES}  --force  --fk
done
